...
void hrtimer_wrapper(intptr_t exinf) {
    ...
    case HRTIMER_RESTART:
        SVC_PERROR(sta_alm(timer->cycid, timer->period));
    	break;
...
int hrtimer_start(struct hrtimer *timer, ktime_t tim, const enum hrtimer_mode mode) {
	...
	if(timer->cycid <= 0) { // Create high resolution alarm handler
        ...
        calm.nfyinfo.nfy.handler.tmehdr = hrtimer_wrapper;
        calm.nfyinfo.nfy.handler.exinf = (intptr_t)timer;
        SVC_PERROR(timer->cycid = acre_alm(&calm));
	}
...
int hrtimer_cancel(struct hrtimer *timer) {
    ...
    assert(timer->cycid > 0);
    ER ercd;
    SVC_PERROR(ercd = stp_alm(timer->cycid));
	return (ercd == E_OK);
}
