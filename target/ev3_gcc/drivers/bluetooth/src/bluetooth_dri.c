...
void bluetooth_qos_task(intptr_t unused) {
	while(1) {
		...
		dly_tsk(BT_HIGH_PRI_TIME_SLICE * 1000U);

		chg_pri(BT_TSK, TPRI_BLUETOOTH_LOW);
		dly_tsk(BT_LOW_PRI_TIME_SLICE * 1000U);
...
#if defined(DEBUG_BLUETOOTH)
        get_tim(&tim2);
        syslog(LOG_NOTICE, "%s(addr%c=NULL) costs %d ms", __FUNCTION__, (addr ? '!' : '='), (tim2 - tim1) / 1000U);
#endif
}
